...
def load(app: Flask):
    ...
    def hint_trade_decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            message = (
                "source=ctfd, event=" + ctfd_config.ctf_name() +
                ",type=hint,success=" + str(result.json['success']) +
                ",challenge=" + challenge.name +
                ",category='" + challenge.category + "'" +
                ",team=\"" + team.name + "\"" +  # Enclose team value in double quotes
                ",user=\"" + user.name + "\"" +  # Enclose user value in double quotes
                ",points=" + str(hint.cost * -1) +
                ",msg=Player " + user.name + " just traded " + str(hint.cost) +
                " points for a hint on challenge " + challenge.name
            )
    ...
    def challenge_attempt_decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if result.json['data']['status'] == "incorrect": 
                ...
                message = (
                    "source=ctfd, event=" + ctfd_config.ctf_name() +
                    ",type=challenge,status=incorrect,challenge='" + challenge.name + "'" +
                    ",category=" + challenge.category +
                    ",team=\"" + team.name + "\"" +  # Enclose team value in double quotes
                    ",user=\"" + user.name + "\"" +  # Enclose user value in double quotes
                    ",points=0" +
                ...
                message = (
                    "source=ctfd, event=" + ctfd_config.ctf_name() +
                    ",type=challenge,status=correct,challenge='" + challenge.name +
                    "',category=" + challenge.category +
                    ",team=" + team.name +
                    ",user=\"" + user.name + "\"" +  # Enclose user value in double quotes
                    ",points=" + str(challenge.value) +
                    ",msg='Team " + team.name + " is the " + str(num_solves) +
                    " to solve challenge " + challenge.name + "'"
                    )
                log("submissions", message)
