...
print("DatadogInviter is starting up. CTFD Token: " + CTFD_TOKEN + " at host: " + CTFD_HOST)
...
def check_start_time():
    ...
    headers = {'content-type': 'application/json', 'Authorization': 'Token ' + CTFD_TOKEN}
    ...
    if response['data']['value'] == '': 
        ...
        print("The CTF has no start time configured. We are not going to manage invitations then.")
        do_invite = False 
        return do_invite 
    ...
    if delta > INVITATION_TIME_MINUTES * 60: 
        print("CTF starts in more than " + str(INVITATION_TIME_MINUTES) + " minutes not inviting yet.")
    else: 
        if delta < INVITATION_CUTOFF_MINUTES * -60: 
            print("CTF started more than " + str(INVITATION_CUTOFF_MINUTES) + " minutes ago. Stopping invitations.")
            do_invite = False
        else:
            print("CTF starts in less than " + str(INVITATION_TIME_MINUTES) + " minutes. Ready to invite.")
            do_invite = True
...
def invite_users():
    ...
    # now, for every team
    for team in teams: 
        print("Checking members for team: " + team['name'] + "(" + str(team['id']) + ")")
        ...
        for user_id in members:
            ...
            if not user['email'] in sent_invitations:
                if invite_user(details['apikey'], details['appkey'], user['email'], role_id):
...
while True: # we run until we're killed
    if check_start_time(): 
        ...
        print("We are not yet at the CTF Start time minus 5 minutes. Not sending invitations yet")
